let twrk = {};
    ...
    lineRotation = function ({ point, long, rotation}) {
        return [{x: point.x, y: point.y}, {x: point.x + Math.sin(rotation)*10, y:point.y + Math.cos(rotation)*10}]; 
    };
    ...
    //drawing
        ...
        let resolution = 0.3; // wie man die auflösung einstellt ist sehr wichtig.. 
        let speed = 0.0001;

        svg.makeLine({parent: dom.svgLayer, id: "lines", cap: "round", stroke: 0.4, color: "blue", d: ""});
        ...
        function loop(time) {
            ...
            for (let y =5; y < 25; y++) {
                for (let x = 0; x < 25; x++) {
                    ...
                    // es wird nochmal der genau gleiche Ablauf gemacht, darum können sie überschrieben werden.
